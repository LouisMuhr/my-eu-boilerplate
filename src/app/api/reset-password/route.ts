...
import { dbHelpers, hashPassword } from "@/lib/db";
...
export async function POST(req: Request) {
  try {
    ...
    if (!token || !password || password.length < 8) {
      return NextResponse.json({ error: "Ungültige Daten oder Passwort zu kurz." }, { status: 400 });
    }

    // 1. Token in der DB suchen
    const resetRequest = (dbHelpers as any).getResetTokenByToken.get(token) as any;
    ...
    return NextResponse.json({ error: "Serverfehler beim Zurücksetzen." }, { status: 500 });
  }
}