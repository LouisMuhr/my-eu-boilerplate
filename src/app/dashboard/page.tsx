...
export default async function DashboardPage() {
  ...
  const stats = [
    { label: "Nutzungsrate", value: "92%", icon: TrendingUp, color: "text-emerald-500" },
    { label: "Aktive Projekte", value: "4", icon: Box, color: "text-indigo-500" },
    { label: "API Status", value: "Online", icon: Activity, color: "text-blue-500" },
  ];
        ...
        <div className="p-8">
          <Link href="/dashboard" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col text-left">
              <span className="font-black text-xl tracking-tighter leading-none uppercase">EU Core</span>
              <span className="text-[10px] text-slate-400 font-bold tracking-widest uppercase mt-1">SaaS Platform</span>
            </div>
          </Link>
        </div>

        <nav className="flex-1 px-4 space-y-1">
          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-4 mb-4">Hauptmenü</div>
          <Link
            href="/dashboard"
          ...
          <div className="flex items-center gap-3 px-4 py-3 text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl font-medium transition-colors cursor-pointer group">
            <Settings className="w-5 h-5 group-hover:rotate-45 transition-transform" /> <span>Einstellungen</span>
          </div>
        </nav>
        ...
        <div className="p-6 border-t border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-3 p-2 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
             ...
             <div className="flex flex-col min-w-0 text-left">
               <span className="text-xs font-bold truncate">{userRow?.name || "Nutzer"}</span>
               <span className="text-[10px] text-slate-400 truncate">{userRow?.email}</span>
             </div>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-50/50 dark:bg-slate-950">
        ...
        {isCanceled && (
          <div className="bg-amber-500 text-white px-8 py-2 flex items-center justify-center gap-3 animate-in slide-in-from-top duration-500">
            ...
            <p className="text-xs font-bold tracking-wide uppercase">
              Abo endet am {formattedDate} – Jetzt verlängern für ununterbrochenen Zugriff
            </p>
          </div>
        )}
        ...
        <header className="h-20 border-b border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-900/70 backdrop-blur-md flex items-center px-8 justify-between sticky top-0 z-30">
          <div className="flex items-center gap-6">
            <h2 className="font-black text-2xl tracking-tight text-slate-800 dark:text-white">Übersicht</h2>
            ...
            >
              <span className={`w-1.5 h-1.5 rounded-full ${isCanceled ? 'bg-amber-400' : subscriptionStatus === 'active' ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`}></span>
              {isCanceled ? "Gekündigt" : subscriptionStatus === "active" ? "Pro Plan" : "Free Plan"}
            </div>
          </div>
        ...
        <div className="p-6 md:p-10 max-w-7xl mx-auto w-full space-y-8">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div className="text-left">
              <h3 className="text-3xl font-black tracking-tight text-slate-900 dark:text-white">Willkommen zurück, {userRow?.name?.split(' ')[0]}!</h3>
              <p className="text-slate-500 mt-1">Hier ist eine Zusammenfassung deiner Ressourcen und deines Status.</p>
            </div>
