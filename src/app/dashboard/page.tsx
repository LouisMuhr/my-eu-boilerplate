    ...
    <div className="min-h-screen bg-[#fafafa] dark:bg-gray-950 flex flex-col md:flex-row font-sans text-gray-900 dark:text-gray-100">
      ...
      <aside className="w-full md:w-72 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex flex-col z-20">
        <div className="p-8 font-black text-2xl tracking-tighter flex items-center gap-3">
           <div className="w-10 h-10 bg-blue-600 rounded-2xl flex items-center justify-center text-white text-xs">EU</div>
           <span>Dashboard</span>
        </div>
        <nav className="flex-1 px-4 space-y-2">
          <Link href="/dashboard" className="flex items-center gap-3 px-4 py-3 text-blue-600 bg-blue-50 dark:bg-blue-900/20 rounded-2xl font-bold border border-blue-100 dark:border-blue-800/50">
          ...
          <div className="bg-gradient-to-r from-amber-500 to-orange-600 text-white px-8 py-3 flex items-center justify-between shadow-lg">
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm font-bold">
                Abo gekündigt. Dein Pro-Zugriff bleibt bis zum <span className="underline">{formattedDate}</span> bestehen.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="p-8 md:p-12 max-w-6xl mx-auto space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
             <div className="bg-white dark:bg-gray-900 p-8 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-sm">
                ...
                <p className="text-[10px] uppercase font-black text-gray-400 tracking-widest">Aktivität</p>
                <p className="text-2xl font-black mt-1">Normal</p>
             </div>
             <div className="bg-white dark:bg-gray-900 p-8 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-sm">
                <ShieldCheck className="w-6 h-6 text-green-600 mb-4" />
                <p className="text-[10px] uppercase font-black text-gray-400 tracking-widest">Sicherheit</p>
                <p className="text-2xl font-black mt-1">DSGVO Konform</p>
             </div>
             <div className="bg-white dark:bg-gray-900 p-8 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-sm">
                <Zap className="w-6 h-6 text-purple-600 mb-4" />
                <p className="text-[10px] uppercase font-black text-gray-400 tracking-widest">Plan-Typ</p>
                <p className="text-2xl font-black mt-1 uppercase">{subscriptionStatus}</p>
             </div>
          </div>
        </div>
      </main>
